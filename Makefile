...
install:
	...
	fpm -s dir -t deb --deb-no-default-config-files -C $(INSTALLDIR) -n $(PKG_NAME) -v $(PKG_VERSION) -p $(DESTDIR)/$(PKG_NAME)_v$(PKG_VERSION)_$(PKG_ARCH).deb --maintainer "$(PKG_MAINTAINER)" --deb-custom-control "$(CONTROL)"
...
deploy:
	...
	curl --silent -u "${APT_API_USERNAME}":${APT_API_PASSWORD}" -X POST file=$(DESTDIR)/$(PKG_NAME)_v$(PKG_VERSION)_$(PKG_ARCH).deb "${APT_API_BASE_URL}/files/xenial" |jq
	curl --silent -u "${APT_API_USERNAME}":"${APT_API_PASSWORD}" -X POST "${APT_API_BASE_URL}/repos/ces-commons/${ARTIFACT_ID}_${VERSION}.deb" |jq
